...
import type { StrokeStyle } from "../../element/types";
...
const parseActor = (actors: { [key: string]: Actor }, containerEl: Element) => {
  ...
  let actorMap: { [key: string]: Actor };
  if (actors instanceof Map) {
    actorMap = Object.fromEntries(actors);
  } else {
    actorMap = actors;
  }
  Object.values(actorMap).forEach((actor, index) => {
    ...
    if (actor.type === "participant") {
      ...
      if (!topNodeElement) {
        throw new Error("Top Node element not found!");
      }
      ...
      if (!topNodeElement.height) {
        throw new Error("Top node element height is null");
      }
      ...
      const topBBox = topRootNode.getBBox();
      const bottomBBox = bottomRootNode.getBBox();
      const circle = topRootNode.querySelector("circle");
      const actorX = circle
        ? Number(circle.getAttribute("cx"))
        : topBBox.x + topBBox.width / 2;
      const topY = topBBox.y + topBBox.height;
      const bottomY = bottomBBox.y;

      const siblingLine =
        topRootNode.previousElementSibling?.tagName === "line"
          ? (topRootNode.previousElementSibling as SVGLineElement)
          : null;
      const parentSiblingLine =
        topRootNode.parentElement?.previousElementSibling?.tagName === "line"
          ? (topRootNode.parentElement
              ?.previousElementSibling as SVGLineElement)
          : null;

      const candidateLines = Array.from(containerEl.querySelectorAll("line"))
        .map((node) => {
        ...
        .filter(({ x1, x2, y1, y2 }) =>
          [x1, x2, y1, y2].every((v) => Number.isFinite(v)),
        )
        .filter(({ x1, x2 }) => Math.abs(x1 - x2) < 0.01)
        .filter(({ x1 }) => Math.abs(x1 - actorX) <= 1)
        .filter(({ y1, y2 }) => {
          const minY = Math.min(y1, y2);
          const maxY = Math.max(y1, y2);
          return minY <= topY + 5 && maxY >= bottomY - 5;
        })
        .sort((a, b) => Math.abs(b.y2 - b.y1) - Math.abs(a.y2 - a.y1));

      const lineNode =
        siblingLine ||
        parentSiblingLine ||
        (candidateLines.length
          ? (candidateLines[0].node as SVGLineElement)
          : null);
      ...
      const startX = Number(lineNode.getAttribute("x1"));
      const startY = Math.max(Number(lineNode.getAttribute("y1")), topY);
      const endX = Number(lineNode.getAttribute("x2"));

      const bottomEllipseNode = bottomNodeElement.find(
        (node): node is Container => node.type === "ellipse",
      );
      const endY = bottomEllipseNode ? bottomEllipseNode.y : bottomY;
      const line = createLineSkeletonFromSVG(
        lineNode,
        startX,
        startY,
...
export const parseMermaidSequenceDiagram = (
  ...
  // Get mermaid parsed data from diagram.db
  // In mermaid 11.x, use diagram.db to access the database
  const mermaidParser = (diagram as any).db;
  const nodes: Array<Node[]> = [];
